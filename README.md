...
| OpenRack / Arivista / Junivista / PacketSense | Arista EOS: the real prompts ((config-if-Et1-3)#, (config-acl-NAME)#, (config-router-bgp)#), % Incomplete command / % Invalid input, `?` with descriptions, ranges without the range keyword, ip access-list with sequence numbers and ip access-group, ip nat source on the outside interface, dhcp server / subnet / range, vrf instance, mlag configuration, interface Vxlan1, ip prefix-list, checkpoints, and the show commands in Arista's shapes (interfaces, ip route, ip interface brief, spanning-tree, port-channel summary, lldp, bgp, ospf, logging, clock, version). The IOS spellings still parse |
