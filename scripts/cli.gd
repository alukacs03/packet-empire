...
const LEARN_TOPICS := [
	...
	[["vrrp", "/interface vrrp"], "VRRP"],
	[["keepalived"], "Keepalived on a pair of servers"],
	[["vtysh", "router ospf6", "ip_forward"], "A server that routes"],
	[["iptables", "nft ", "ufw", "/ip firewall filter"], "The host's own firewall"],
	[["route-map", "address-family", "set local-preference", "set as-path"], "Route maps and address families"],
	[["ip helper-address", "/ip dhcp-relay"], "Leases across a router"],
	[["spanning-tree", "/interface bridge port monitor", "bpduguard", "portfast"], "Spanning tree"],
	[["router ospf", "/routing ospf", "ospf"], "OSPF"],
	[["router bgp", "/routing bgp", "bgp", "remote-as"], "BGP and the internet"],
	[["ip nat", "/ip firewall nat", "masquerade"], "NAT"],
	[["dhcp", "dhclient", "/ip dhcp", "isc-dhcp-server"], "DHCP"],
	[["arp", "ip neigh", "/ip arp"], "ARP"],
