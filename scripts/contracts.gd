...
const DIALECT_HINTS := {
	...
	"lock_it_down": {
		...
		"eos": [
			"enable",
			"configure terminal",
			"ip access-list VAULT",
			"10 deny ip 172.16.1.0/24 host 172.16.2.20",
			"20 permit ip any any",
			"exit",
			"interface Ethernet1     (the office-facing leg)",
			"ip access-group VAULT in",
			"end",
	...
	"hide_the_internals": {
		...
		"eos": [
			...
			"10 permit ip 10.3.0.0/24 any",
			"exit",
			"interface Ethernet1     (the uplink: NAT is written on the outside interface)",
			"ip nat source dynamic access-list NAT-ACL overload",
			"end",
	...
	"overlay_tenant": {
		...
		"after": "Run the Vxlan1 block on both leaves with each other's VTEP address in the flood list. The unmapped VLAN 71 must stay isolated.",
		"eos": [
			"enable",
			"configure terminal",
			"vlan 70",
			"interface Loopback1",
			"ip address <this leaf's VTEP address>/32",
			"interface Vxlan1",
			"vxlan source-interface Loopback1",
			"vxlan vlan 70 vni 7000",
			"vxlan flood vtep <the other leaf's VTEP address>",
			"vxlan evpn",
			"end",
			"show vxlan vtep",
		],
	...
	"pull_either_cable": {
		...
		"eos": [
			...
			"vlan 4094",
			"interface Vlan4094",
			"ip address <this switch's peer address>/30",
			"interface Ethernet8",
			"switchport mode trunk",
			"exit",
			"mlag configuration",
			...
			"peer-address <the other switch's peer address>",
			"peer-link Ethernet8",
			"exit",
			"interface Ethernet1",
			"mlag 1",
	...
	"two_transits": {
		...
		"eos": [
			...
			"neighbor 100.64.0.1 prefix-list ONLY-DEFAULT in",
			"neighbor 100.65.0.1 prefix-list ONLY-DEFAULT in",
			"end",
			"show ip bgp",
		],
	},
...
static func _campaign() -> Array:
	return [
		...
		},
		{
			...
			"reward": 3200,
			"brief": "Two acquired companies both built their office on 10.0.0.0/24 and neither will renumber. Route both from one router anyway: give the router a VRF per tenant ('vrf instance alfa', 'vrf instance beta'), put each tenant's interface into its table ('vrf alfa' under the interface) and address both legs 10.0.0.1/24. A host in each tenant must reach its own 10.0.0.1, and the global table must know nothing about either. 'show vrf' lists the tables and 'ping vrf alfa 10.0.0.50' tests one of them.",
			"reqs": [
		...
		},
		{
			...
			"reward": 5200,
			"brief": "Turul Mobil have outgrown a stretched VLAN and want their segment on two different leaf switches with a routed network in between: no trunk, no cable between the leaves. Build the underlay first (an SVI on each leaf, a router between them, and each leaf able to ping the other's address), then put the tenant on top with VXLAN, on each leaf: a Loopback with the VTEP address, then 'interface Vxlan1' with 'vxlan source-interface Loopback1', 'vxlan vlan 70 vni 7000' and 'vxlan flood vtep <the other leaf's VTEP address>' on both. Their hosts are 192.168.70.10 and 192.168.70.11. Put a second tenant in VLAN 71 on one leaf, unmapped, and prove it cannot see any of it. Then turn on 'vxlan evpn' so the leaves tell each other what they have instead of flooding.",
			"reqs": [
