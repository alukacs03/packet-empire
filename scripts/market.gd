...
const KINDS := {
	...
	"public_hosting": {"load": 600, "base": 220, "spread": 160,
		"brief": "We want a public web presence: host our server at %s and make it reachable from the internet: your upstream must be able to reach it, so think a BGP announcement, or static NAT since it must accept inbound.",
		"costs": "A server + working transit (uplink, BGP session, prefix announced). Premium tier."},
	"redundant_gw": {"load": 400, "base": 260, "spread": 180,
		"brief": "After the last outage we demand a redundant gateway: virtual IP %s served by two routers (VRRP), and our server must use it as its default gateway.",
