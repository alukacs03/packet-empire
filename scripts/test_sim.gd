...
static func run() -> int:
	...
	check(v1.exec("show run").contains("vrrp 1 preempt disabled"), "vrrp: show run keeps the preempt setting")
	v1.exec("conf t")
	v1.exec("int et1")
	...
	Scenarios.finish(false)
	# the inherited fabric: three configuration faults, no cable wrong
	Scenarios.start(Scenarios.all()[4])
	check(not Scenarios.solved(), "scenario: the inherited fabric starts with its faults")
	var fab_r1 := Sim._ip_owner("10.63.1.2")
	var fab_r1b := Sim._ip_owner("10.63.1.3")
	check(fab_r1 != null and Sim.ospf_neighbors(fab_r1).is_empty(), "scenario: a network statement short of the transit link means no neighbour")
	var fab_cli := CLI.new_session(fab_r1)
	fab_cli.exec("en")
	fab_cli.exec("conf t")
	fab_cli.exec("router ospf 1")
	check(fab_cli.exec("network 10.63.0.0 0.0.255.255 area 0") == "", "scenario: the IOS wildcard form of the network statement is accepted")
	fab_cli.exec("end")
	check(not Sim.ospf_neighbors(fab_r1).is_empty(), "scenario: covering the transit link brings the adjacency up")
	var fab_b_cli := CLI.new_session(fab_r1b)
	fab_b_cli.exec("en")
	fab_b_cli.exec("conf t")
	...
	Scenarios.finish(false)
	# the webshop's bad Friday: a dead pool member and NAT on the wrong leg
	Scenarios.start(Scenarios.all()[5])
	check(not Scenarios.solved(), "scenario: Friday starts broken")
	var fri_web2 := Sim._ip_owner("10.64.0.12")
	var fri_rtr := Sim._ip_owner("10.64.0.1")
	fri_web2.ifaces[0].admin_down = false
	fri_web2.ifaces[0].enabled = true
	Game.topology_changed.emit()
	var fri_cli := CLI.new_session(fri_rtr)
	fri_cli.exec("en")
	fri_cli.exec("conf t")
	fri_cli.exec("interface Ethernet1")
	fri_cli.exec("ip nat outside")
	fri_cli.exec("interface Ethernet2")
	fri_cli.exec("ip nat inside")
	fri_cli.exec("exit")
	fri_cli.exec("no ip nat inside source")
	fri_cli.exec("ip nat inside source list 1 interface Ethernet1 overload")
	fri_cli.exec("end")
	check(Scenarios.solved(), "scenario: a live pool and NAT on the right leg save the sale")
	Scenarios.finish(false)
	...
	css.exec("conf t")
	check(css.exec("enable secret hunter2") == "", "enable secret: accepted")
	css.exec("end")
	var t28_run := css.exec("show running-config")
	check(t28_run.contains("enable secret sha512 ") and not t28_run.contains("hunter2"), "enable secret: the running-config prints the hash, never the word")
	var t28_s := CLI.new_session(cs_sw)
	check(t28_s.exec("enable").contains("Password") and t28_s.mode == "exec", "enable secret: enable asks for the password")
	check(t28_s.exec("wrong") == "% Access denied\n" and t28_s.mode == "exec", "enable secret: a wrong password is refused")
	t28_s.exec("enable")
	check(t28_s.exec("hunter2") == "" and t28_s.mode == "priv", "enable secret: the right password enters privileged mode")
	var t28_hash := String(cs_sw.services["enable_secret"])
	t28_s.exec("conf t")
	check(t28_s.exec("enable secret sha512 %s" % t28_hash) == "" and cs_sw.services["enable_secret"] == t28_hash, "enable secret: the hashed line pastes back unchanged")
	t28_s.exec("no enable secret")
	t28_s.exec("end")
	check(not cs_sw.services.has("enable_secret") and CLI.new_session(cs_sw).exec("enable") == "", "enable secret: removed, enable is open again")
	...
	# FRR's routes reach the kernel table: a router on the LAN advertises a far subnet, ip route shows it as proto ospf
	var t12_rf := Game.new_device("rtr-edge")
	t12_rack.slots[2] = t12_rf
	Game.connect_ifaces(t12_rf.ifaces[0], t12_sw.ifaces[1])
	Game.connect_ifaces(t12_rf.ifaces[1], t12_sw.ifaces[2])
	Game.add_ip(t12_rf.ifaces[0], "10.78.0.1/24")
	Game.add_ip(t12_rf.ifaces[1], "10.78.1.1/24")
	var t12_rfs := CLI.new_session(t12_rf)
	t12_rfs.exec("en")
	t12_rfs.exec("conf t")
	t12_rfs.exec("router ospf 1")
	t12_rfs.exec("network 10.78.0.0/16 area 0")
	t12_rfs.exec("end")
	Sim.flush_learned_state()
	...
	# the bond and the sub-interface the way iproute2 shows them
	var t33_eth1 := t17_l.exec("ip link show eth1")
	check("SLAVE" in t33_eth1 and "master bond0" in t33_eth1, "ip link: an enslaved port says SLAVE and names its master")
	check(t17_l.exec("ip -br link show eth0").split("\n", false).size() == 1, "ip -br link show eth0 prints one line, not lo as well")
	check(t17_l.exec("ip link set eth1 master bond3").contains("Cannot find device"), "ip link: enslaving into a bond that was never created is refused")
	check(t17_l.exec("ip link del bond0") == "" and t17_s.ifaces[1].lag == 0, "ip link del bond0 frees its members")
	t17_l.exec("ip link set eth1 up")
	check(t17_l.exec("ip link add link eth0 name eth0.30 type vlan id 30") == "", "ip link: a vlan sub-interface for the next checks")
	var t33_sub := t17_l.exec("ip link show eth0.30")
	check("eth0.30@eth0" in t33_sub and "noqueue" in t33_sub and "vlan protocol" not in t33_sub, "ip link: a vlan device is named with its link and hides the detail line without -d")
	...
	check(t17_l.exec("iptables -L NOSUCH").contains("No chain/target/match"), "iptables -L on an unknown chain says so")
	t17_l.exec("iptables -F INPUT")
	...
	# EOS: completion in a sub-mode, trunk lists, preempt, session diffs, startup rendering
	t35.exec("interface Ethernet4")
	check("route" in t35.complete("ip ro") and "write" in t35.complete("wr"), "eos: completion inside interface mode offers what exec accepts")
	t35.exec("switchport mode trunk")
	check(t35.exec("show interfaces trunk").contains("All"), "eos: an unrestricted trunk prints All")
	check(t35.exec("switchport trunk allowed vlan remove 10") == "" and t35.exec("show interfaces trunk").contains("1-9,11-4094")
		and t35.exec("show running-config").contains("switchport trunk allowed vlan 1-9,11-4094"), "eos: remove from All leaves every other VLAN, printed as ranges")
	check(t35.exec("switchport trunk allowed vlan none") == "" and t35.exec("show running-config").contains("switchport trunk allowed vlan none"), "eos: none is spelled none")
	t35.exec("switchport trunk allowed vlan all")
	t35.exec("exit")
	t35.exec("interface Ethernet5")
	t35.exec("no switchport")
	t35.exec("ip address 10.35.5.1/24")
	check(t35.exec("vrrp 1 ipv4 10.35.5.254") == "" and t35.exec("vrrp 1 preempt disabled") == "" and t35.exec("show running-config").contains("vrrp 1 preempt disabled")
		and not bool(t35_sw.ifaces[4].vrrp["preempt"]), "eos: vrrp preempt disabled is accepted and printed the 4.2x way")
	t35.exec("exit")
	t35.exec("exit")
	t35.exec("configure session s35")
	t35.exec("interface Ethernet6")
	t35.exec("switchport mode trunk")
	t35.exec("exit")
	var t35_diff := t35.exec("show session-config diffs")
	check(t35_diff.contains("interface Ethernet6") and t35_diff.contains("+   switchport mode trunk"), "eos: the session diff sees a line another port already has")
	t35.exec("abort")
	t35.exec("configure")
	t35.exec("interface Ethernet3")
	t35.exec("description live-desc")
	t35.exec("exit")
	...
	Sim.flush_learned_state()
	check(Sim.ping(t17_s, "10.79.0.2")["ok"], "fw: flushing OUTPUT lets it send again")
	t17_rs.exec("/ip firewall filter remove 2")  # the icmp input drop from above would eat the echo reply; out of the way first
	Sim.flush_learned_state()
	check(Sim.ping(t17_r, "10.79.0.2")["ok"], "ros fw: with the input drop gone the router pings the host")
	check(t17_rs.exec("/ip firewall filter add chain=output dst-address=10.79.0.2/32 action=drop") == "" and not Sim.ping(t17_r, "10.79.0.2")["ok"], "ros fw: chain=output stops the router's own traffic")
	t17_rs.exec("/ip firewall filter remove 2")  # the output rule took the freed row
	Sim.flush_learned_state()
	check(Sim.ping(t17_r, "10.79.0.2")["ok"], "ros fw: removing it lets the router send again")
	...
	var t27_rtr := Game.new_device("rtr-edge")  # a DHCP server lives on a router
	var t27_es := CLI.new_session(t27_rtr)
	t27_es.exec("en")
	t27_es.exec("conf t")
	t27_es.exec("dhcp server")
	t27_es.exec("subnet 10.79.5.0/24")
	t27_es.exec("range 10.79.5.10 10.79.5.20")
	t27_es.exec("lease time 0 2 0")
	t27_es.exec("end")
	check(t27_es.exec("show running-config").contains("dhcp server\n   lease time 0 2 0\n"), "lease: the EOS running-config prints the lease time under the dhcp server block")
	t27_rtr.services.erase("dhcp")
